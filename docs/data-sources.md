...
調査日: 2026-07-14
調査環境: macOS 26.5.2 (Darwin 25.5.0, arm64) / Xcode 26.2 / Swift 6.2.3。Windows版は同じ匿名化fixtureとWindowsネイティブCLIのパス仕様を使用する。

## Windows版のパス差分

| Provider | 読み取り先 |
|---|---|
| Claude Code | `CLAUDE_CONFIG_DIR`があればその`projects`、なければ`%USERPROFILE%\.claude\projects` |
| Claude OAuth（任意） | Claude homeの`.credentials.json`にある`claudeAiOauth`。トークンはメモリ上だけで使用 |
| Codex | `CODEX_HOME`があればその`sessions`、なければ`%USERPROFILE%\.codex\sessions` |
| Copilot CLI | `%USERPROFILE%\.copilot\session-state\*\events.jsonl` |

Windows版もCodexの`auth.json`を開かず、セッションログの存在と可読性だけで状態を判定する。macOSとWindowsでDBを共有・同期しない。
...
## 2.2 Copilot CLI — session shutdown JSONL 【Windows版で採用】

| 項目 | 内容 |
|---|---|
| パス | `%USERPROFILE%\.copilot\session-state\<sessionId>\events.jsonl` |
| 対象 | `type == "session.shutdown"`の`data.modelMetrics.<model>.usage` |
| 取得値 | `inputTokens`、`outputTokens`、`cacheReadTokens`、`cacheWriteTokens`、モデル名、日時 |
| 利用率・リセット時刻 | ローカルログには無いため表示しない |

shutdownの値はセッション・モデル単位の累積値なので、前回値との差分だけをイベントとして保存する。カウンターが減少した場合は再起動とみなし、現在値を全量として扱う。`cacheReadTokens`は`inputTokens`に含まれるため、実作業量は`totalTokens - cachedInputTokens`で求め、二重計上しない。
...
| Copilot CLI（Windows） | `%USERPROFILE%\.copilot\session-state\*\events.jsonl` | `.localLog` | トークン内訳・モデル・履歴 |

通常は有効なプロバイダーのログディレクトリだけを読む。OAuth連携を明示的に有効にした場合だけ、macOSではKeychain、WindowsではClaude homeの`.credentials.json`を読む。
平文の認証ファイルには一切触れない。
